...
export class AuthService {
    ...
    private readonly passwordHmacSecret: string;
    ...
    constructor(
        ...
        this.passwordHmacSecret = this.configService.get<string>('PASSWORD_HMAC_SECRET') || this.jwtSecret;
    }
    ...
    > {
        try {
            ...
            const failedAttempts = await this.rawCacheService.get<number>(lockoutKey);
            if (failedAttempts && failedAttempts >= 10) {
                const ttl = await this.rawCacheService.getTtl(lockoutKey);
                await this.emitFailedLoginAttempt(
                    username,
                    password,
                    ip,
                    userAgent,
                    `Account locked. Too many failed attempts. Retry in ${Math.ceil((ttl || 0) / 60)} minutes.`,
                );
                return fail({
                    ...ERRORS.FORBIDDEN,
                    message: `Account temporarily locked. Try again in ${Math.ceil((ttl || 0) / 60)} minutes.`,
                });
            }
            ...
            if (!statusResponse.response.isLoginAllowed) {
                await this.incrementLockoutCounter(username);
                await this.emitFailedLoginAttempt(
                    username,
                    password,
                    ip,
            ...
            await this.resetLockoutCounter(username);
            ...
            const tokenHash = createHmac('sha256', this.jwtSecret)
                .update(refreshToken)
                .digest('hex');
            await this.rawCacheService.set(`refresh:${tokenHash}`, admin.response.uuid, 604800);
    ...
    > {
        try {
            ...
            const tokenHash = createHmac('sha256', this.jwtSecret)
                .update(refreshToken)
                .digest('hex');
            await this.rawCacheService.set(`refresh:${tokenHash}`, createAdminResponse.response.uuid, 604800);
    ...
    private async processOAuth2Callback(
        ...
        if (!allowedEmails.includes(emailResult.email)) {
            await this.emitFailedLoginAttempt(
                emailResult.email,
                '–',
                ip,
                userAgent,
                `${provider} email not in allowed list.`,
            );
            return FAIL;
        }
    ...
    private async hashPassword(plainPassword: string): Promise<string> {
        const hmacResult = this.applySecretHmac(plainPassword, this.passwordHmacSecret);
    ...
    private async incrementLockoutCounter(username: string): Promise<void> {
        const lockoutKey = `lockout:${username}`;
        const current = await this.rawCacheService.get<number>(lockoutKey);
        await this.rawCacheService.set(lockoutKey, (current || 0) + 1, 900); // 15 min TTL
    }

    private async resetLockoutCounter(username: string): Promise<void> {
        await this.rawCacheService.del(`lockout:${username}`);
    }
    ...
    public async verifyPasskeyAuthentication(
        ...
        try {
            ...
            // Generate refresh token
            const refreshToken = nanoid(64);
            const refreshTokenKey = `refresh:${admin.response.uuid}`;
            await this.rawCacheService.set(refreshTokenKey, refreshToken, 604800);
    ...
    public async refreshToken(
        ...
        try {
            // Look up refresh token by its hash
            const tokenHash = createHmac('sha256', this.jwtSecret)
                .update(refreshToken)
                .digest('hex');
            const refreshKey = `refresh:${tokenHash}`;

            const adminUuid = await this.rawCacheService.get<string>(refreshKey);
            if (!adminUuid) {
                return fail(ERRORS.FORBIDDEN);
            }
            ...
            const newTokenHash = createHmac('sha256', this.jwtSecret)
                .update(newRefreshToken)
                .digest('hex');
            const newRefreshKey = `refresh:${newTokenHash}`;

            await this.rawCacheService.del(refreshKey);
            await this.rawCacheService.set(newRefreshKey, adminUuid, 604800);
